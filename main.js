...
fetch("https://6821ad3d259dad2655b02055.mockapi.io/posts/posts")
  ...
  .then((data) =>
    data.forEach((post) => {
      ...
      users.push(post.username);
    })
  );

submitBtn.addEventListener("click", () => {
  console.log(users.includes(username.value));
  if (username.value) {
    if (users.includes(username.value)) {
      errorMsg.innerText = "اسم المستخدم موجود بالفعل";
      return;
    }
  } else {
    errorMsg.innerText = "أدخل اسم المستخدم";
    return;
  }
  if (postImg.value == false) {
    errorMsg.innerText = "أدخل رابط الصورة";
    return;
  }
  if (postContent.value == false) {
    errorMsg.innerText = "أدخل محتوى البوست ";
    return;
  }
  fetch("https://6821ad3d259dad2655b02055.mockapi.io/posts/posts", {
    method: "POST",
